...
def create_boxes():
    ...
    def save_feedback():
        ...
        if full_name_entry.get() == "":
            ...
            excel_filepath = pathlib.Path.home() / 'Desktop'

            try:
                existing_df = pd.read_excel(f'{excel_filepath}/feedback_data.xlsx', sheet_name='FeedbackData')
                updated_df = pd.concat([existing_df, df], join='outer',ignore_index=False)
                updated_df.to_excel(f'{excel_filepath}/feedback_data.xlsx', index=False, sheet_name='FeedbackData', engine='openpyxl')
                
            except FileNotFoundError:
                df.to_excel(f'{excel_filepath}/feedback_data.xlsx', index=False, sheet_name='FeedbackData', engine='openpyxl')
    ...
    def light_mode():
        ...
        light=ImageTk.PhotoImage(Image.open(resource_path('icons\\light_light.png')).resize((20,20),Image.BICUBIC))
        dark=ImageTk.PhotoImage(Image.open(resource_path('icons\\light_dark.png')).resize((20,20),Image.BICUBIC))
...
# pyinstaller --onefile --noconsole --icon=your_icon.ico --hidden-import=module_name --hidden-import=module2_name your_script.py
# pyinstaller --onefile --noconsole --icon="D:\desktop_application\FeedBack\icons\feedback.ico" --hidden-import="C:\Users\ASUS\anaconda3\Lib\site-packages\customtkinter" "feedback_system.py"
# pyinstaller --name FeedbackSystem --onefile --windowed --icon=icon.ico feedback_system.py